...
export default function Histogram<T>(props: HistogramProps<T>) {
  ...
  return (
    <div style={histWrapperStyle}>
      {props.s.hist.map((h, i) => {
        ...
        return (
          <div
            ...
            style={{
              ...histBinStyle,
              ...(dense || i === lastBin ? histBinStyleDense : {}),
      ...
      {props.s.min != null && (
        <span style={histMinSpanStyle} title={props.s.format(props.s.min)}>
          {props.s.format(props.s.min)}
        </span>
      )}
      {props.s.max != null && (
        <span style={histMaxSpanStyle} title={props.s.format(props.s.max)}>
          {props.s.format(props.s.max)}
        </span>
      )}
    </div>
  );
